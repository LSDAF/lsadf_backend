...
public class StageWebSocketEventHandler implements WebSocketEventHandler {
  ...
  public void handleEvent(WebSocketSession session, Event event) throws Exception {
    ...
    UpdateCacheStageCommand command =
        new UpdateCacheStageCommand(
            stageEvent.getGameSaveId(),
            stageEvent.getPayload().currentStage(),
            stageEvent.getPayload().maxStage(),
            stageEvent.getPayload().wave());

    stageCommandService.updateCacheStage(command);

    sendAck(session, stageEvent);
  }
  ...
  private void sendAck(WebSocketSession session, StageUpdateWebSocketEvent event) throws Exception {
