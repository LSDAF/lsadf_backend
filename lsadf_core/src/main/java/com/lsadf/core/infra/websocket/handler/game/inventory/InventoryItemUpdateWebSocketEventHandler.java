...
public class InventoryItemUpdateWebSocketEventHandler implements WebSocketEventHandler {
  ...
  public void handleEvent(WebSocketSession session, WebSocketEvent event) throws Exception {
    ...
    try {
      ...
      log.info("Handling inventory item update for clientId: {}, payload: {}", clientId, payload);
      Item item = inventoryService.updateItemInInventory(gameSaveId, clientId, payload);
      log.info("Item updated successfully with clientId: {}", item.getClientId());

      sendAck(session, event);
    } catch (Exception e) {
      log.error("Error processing inventory item update", e);
      ErrorWebSocketEvent errorEvent =
          eventFactory.createErrorEvent(
              event, "Error while updating inventory item: " + e.getMessage());
