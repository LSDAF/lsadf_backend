...
@Getter
public class CurrencyUpdateWebsSocketEvent extends AWebSocketEvent {
  private final CurrencyRequest payload;
  private final UUID gameSaveId;

  protected CurrencyUpdateWebsSocketEvent(
      EventType eventType,
      UUID sessionId,
      UUID messageId,
      UUID userId,
      UUID gameSaveId,
      CurrencyRequest payload) {
    super(eventType, sessionId, messageId, userId);
