...
class WebSocketEventHandlerRegistryTests {
  ...
  void setUp() {
    ...
    registry = new WebSocketEventHandlerRegistry(List.of(handler1, handler2));
  }
  ...
  void shouldThrowExceptionWhenNoHandlerFound() {
    when(event.getEventType()).thenReturn(WebSocketEventType.STAGE_UPDATE);
  ...
  void shouldReturnFalseWhenHandlerDoesNotExist() {
    assertFalse(registry.hasHandler(WebSocketEventType.STAGE_UPDATE));
  }
