...
class StageWebSocketEventHandlerTests {
  ...
  void shouldHandleStageUpdateEvent() throws Exception {
    ...
    ArgumentCaptor<UpdateCacheStageCommand> commandCaptor =
        ArgumentCaptor.forClass(UpdateCacheStageCommand.class);
    verify(stageCommandService).updateCacheStage(commandCaptor.capture());

    UpdateCacheStageCommand command = commandCaptor.getValue();
    assertEquals(gameSaveId, command.gameSaveId());
    assertEquals(5L, command.currentStage());
    assertEquals(10L, command.maxStage());
    assertEquals(3L, command.wave());
